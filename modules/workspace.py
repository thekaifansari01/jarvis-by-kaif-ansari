...
class WorkspaceManager:
    def __init__(self, base_path="Data/Jarvis_Workspace"):
        self.base_path = Path(base_path).resolve() # Secure base path
    ...
    def _init_workspace(self):
        """Creates folders, runs cleanup, and syncs registry on startup."""
    ...
    def _get_safe_filename(self, directory, filename):
        """🛡️ FIX 2: Prevents overwriting by adding (1), (2) to duplicates."""
    ...
    def _is_safe_path(self, directory, filename):
        """🛡️ FIX 3: Prevents Path Traversal attacks (e.g., ../../main.py)."""
        try:
            # Get absolute resolved path
            target_path = (directory / Path(filename)).resolve()
            # Check if the target path is strictly inside the intended directory
            return str(target_path).startswith(str(directory.resolve()))
        except Exception:
            return False

    def sync_registry(self):
        """🛡️ FIX 1 (Upgraded): TWO-WAY Sync. Removes ghosts AND adds manual files."""
        registry = self._load_registry()
        # Dictionary to track which files are currently in registry
        existing_records = {f["filename"]: f for f in registry.get("files", [])}
        
        valid_files = []
        changed = False
        
        # Step A: Scan Disk & Add to Registry (Finds manually pasted files)
            ...
            for file_path in folder_path.iterdir():
                if file_path.is_file() and file_path.name != "registry.json":
                    ...
                    if filename not in existing_records:
                        ...
                        # File is safe, keep its old record
                        valid_files.append(existing_records[filename])
                        # Remove from tracking dict
                        del existing_records[filename]

        # Step B: Any records left in existing_records are 'Ghost Files'
        if existing_records:
            changed = True
    ...
    def cleanup_temp(self):
        """🗑️ Deletes files in Temp folder older than 24 hours."""
        now = time.time()
        cleaned = False
    ...
    def get_storage_status(self):
        """💾 Calculates actual folder size to monitor the 10GB limit."""
    ...
    def add_file_record(self, filename, folder_name, description):
        """Logs a new file into the registry."""
        registry = self._load_registry()
    ...
    def get_safe_filepath(self, folder_name, original_filename):
        """🔒 Helper for executor.py to get a secure and non-overwriting path."""
        target_dir = getattr(self, f"{folder_name.lower()}_dir", self.vault_dir)
    ...
    def get_workspace_context(self):
        """Returns a string summary of files and storage status for Jarvis."""
        self.sync_registry() # Sync instantly before telling Jarvis
    ...
    def _get_safe_filename(self, directory, filename):
        """Prevents overwriting by adding (1), (2) to duplicates."""
        base_name, ext = os.path.splitext(filename)
        counter = 1
        safe_name = filename
        while (directory / safe_name).exists():
    ...
    def get_safe_filepath(self, folder_name, original_filename):
        """Helper for executor.py to get a secure and non-overwriting path."""
        # Fix destination to ONLY our 3 allowed folders
        if folder_name.lower() == "creations": target_dir = self.creations_dir
        elif folder_name.lower() == "temp": target_dir = self.temp_dir
        else: target_dir = self.vault_dir # Default fallback
