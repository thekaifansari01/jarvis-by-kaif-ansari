...
from core.brain.config import (
    GEMINI_EMBEDDING_MODEL, EMBEDDING_DIM, GROQ_SUMMARY_MODEL
)
...
class ContextMemory:
    def __init__(self, memory_path="Data/jarvis_memory", rag_base_path="Data/RAG"):
        ...
        self._init_collections()   # <-- NEW: No deletion, just get or create
        
        # ✅ FIX: Run RAG indexing in background to avoid blocking startup
        if self.rag_base_path.exists():
            threading.Thread(target=self._index_rag_files, daemon=True).start()

    # ============================================================
    # 🔥 FIXED: Collections are NEVER deleted. Just get or create.
    # ============================================================
    def _init_collections(self):
        """
        Initialize ChromaDB collections for chat history and RAG documents.
        Does NOT delete any existing data – preserves all memory across restarts.
        """
        try:
            ...
            logger.info("📀 Collections ready (existing memory preserved)")
        except Exception as e:
            logger.error(f"Failed to initialize ChromaDB collections: {e}")
            raise

    # Legacy method kept for compatibility (does nothing destructive)
    def _recreate_collections_if_needed(self):
        """
        Deprecated: kept for backward compatibility. Now just calls _init_collections.
        """
    ...
    def _smart_chunk_text(self, text, max_chars=2000):
    ...
    # ============================================================
    # 🔥 FIX: Run insights extraction in background thread
    # ============================================================
    def _async_extract_insights(self, message):
        """Start background thread for insight extraction to avoid blocking main flow."""
    ...
    # ============================================================
    # 🔥 NEW: Background embedding & storage
    # ============================================================
    def _async_add_message(self, role, message):
        """Background thread: generate embedding and store in ChromaDB."""
        try:
            embedding = self.get_embedding(message, "search_document")
            if not embedding:
                logger.error(f"Failed to generate embedding for {role} message")
                return
            ...
            # Summarization check (lightweight, but done in background)
            if self.chat_collection.count() > 500:
                self._summarize_old_messages()
                
        except Exception as e:
            logger.error(f"Background message storage failed: {e}")

    # 🚀 UPDATED `add_message` - NON-BLOCKING
    def add_message(self, role, message):
        if not message or not message.strip(): return
        
        # Determine if we should ignore based on the user's extracted message
        ignore_words = ["ok", "okay", "yes", "no", "thanks", "thank you", "clear", "done", "nice", "cool", "hmm", "acha"]
        
        user_text = message
        if role == "CONVERSATION":
            # Extract the user's portion to evaluate for insight extraction
            try:
                user_text = message.split("\nJarvis:")[0].replace("User: ", "").strip()
            except:
                pass
                
        # We only skip tracking trivial single-word responses
        if user_text.lower().strip() in ignore_words:
            return

        if role == "USER" or role == "CONVERSATION":
            self._track_session_state(user_text)
            # Run insight extraction in background – non-blocking
            self._async_extract_insights(user_text)

        # Start background thread for embedding + storage (non-blocking)
        threading.Thread(target=self._async_add_message, args=(role, message), daemon=True).start()
