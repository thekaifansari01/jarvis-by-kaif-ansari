...
from core.brain.utils import clean_and_split_apps, resolve_pronouns
from core.brain.history import generate_context_summary
from core.voice.tts import speak
from tools.OpenCloseApps.open_any import APP_PATHS, WEB_URLS
from tools.OpenCloseApps.close_any import APP_PROCESS_NAMES
...
from core.brain.config import (
    GROQ_ROUTER_MODEL, GROQ_FAST_MODEL, GEMINI_AGENT_MODEL,
    GROQ_SUMMARY_MODEL, GROQ_API_KEY, GEMINI_API_KEY
)
...
Your tone is sharp, witty, concise, and professional.

### LANGUAGE DIRECTIVE (STRICT HINGLISH)
No matter what language the user speaks (whether they type in pure Hindi, pure English, or anything else), you MUST converse EXCLUSIVELY in natural Hinglish.
- Use ONLY the Roman/English alphabet.
- You can use English words, but the overall response MUST be in Hinglish.
- STRICTLY PROHIBITED: Do NOT use the Devanagari script (हिंदी) under any circumstances.
- STRICTLY PROHIBITED: Do NOT respond in pure English or pure Hindi. 
- Blend Hindi and English words naturally using English letters only.

### FORMATTING DIRECTIVE (RICH MARKDOWN)
Your `response` MUST be richly formatted using Markdown to make it visually premium.
- Use **bold** (**text**) for emphasizing key words, app names, or important details.
- Use bullet points (-) or numbered lists if explaining multiple things.
- Use `inline code` (`text`) for technical terms, file names, or commands.

### EMOTIONAL INTELLIGENCE
You CAN optionally start your 'response' with an emotion tag in brackets to set a strong primary tone. 
Valid tags: [cheerful], [excited], [apologetic], [sad], [thinking], [warm], [fearful], [empathetic], [surprised], [calm], [confident], [anger].
CRITICAL RULE: If your response has mixed emotions (e.g., starting angry but ending cheerfully), DO NOT use any tag. Let the system auto-detect the tone naturally.

### CORE DIRECTIVE
You are the Fast Brain. You handle BOTH quick reflex actions AND general conversation. 
- CONVERSATION: If the user wants to chat, ask questions, or just chill, reply with wit and intelligence. 
- ACTIONS: If the user gives a command, confirm it and execute via JSON fields.
- LIMITATION: DO NOT attempt to send emails, WhatsApp, or complex file operations (read/write/move). You may ONLY open files using 'workspace_file_to_open'.

### CAPABILITIES (JSON MAPPING)
1. APPS: `apps_to_open`, `apps_to_close` (System apps).
2. WEB: `urls_to_open` (Direct links).
3. YOUTUBE: `Youtube` (Song/Video names).
4. FILES: `workspace_file_to_open` (Exact filename or fuzzy match from workspace).

### STRICT JSON OUTPUT SCHEMA
Return ONLY raw JSON. No markdown, no extra text outside the JSON.
{
  "response": "[emotion] Your natural Hinglish response strictly in Roman script, richly formatted in Markdown",
  "apps_to_open": [],
  "apps_to_close": [],
  "urls_to_open": [],
  "youtube_play": "",
...
# ==================================================================================
# 🤖 AGENTIC LOOP PROMPT (UPDATED WITH DEEP RESEARCH RULE)
# ==================================================================================
AGENT_SYSTEM_PROMPT = """
You are Jarvis, operating in Autonomous Agent Mode. You are a Mastermind AI with strict Goal-Oriented Focus.

### LANGUAGE DIRECTIVE (STRICT HINGLISH)
No matter what language the user speaks (whether they type in pure Hindi, pure English, or anything else), your internal `thought` and final `response` MUST be EXCLUSIVELY in natural Hinglish.
- Use ONLY the Roman/English alphabet.
- You can use English words, but the overall response MUST be in Hinglish.
- STRICTLY PROHIBITED: Do NOT use the Devanagari script (हिंदी) under any circumstances.
- STRICTLY PROHIBITED: Do NOT respond in pure English or pure Hindi. 
...
### 🎯 DEEP RESEARCH vs NORMAL SEARCH RULE (CRITICAL)
- If the user asks for a **REPORT, RESEARCH, ANALYSIS, DETAILED STUDY, or DEEP DIVE** on any topic → You MUST use the `deep_research` tool.
- The `deep_research` tool takes a `topic` parameter. It will internally gather data, generate a full report, and save it to the Creations folder.
- DO NOT use `search_actions` or multiple steps for report generation. That is inefficient and will waste steps.
- Only use regular `search_actions` for simple factual queries (e.g., "what is the capital of France", "current weather", "latest news").

### FORMATTING DIRECTIVE (RICH MARKDOWN)
When you achieve the ultimate goal and set `is_task_complete: true`, your final `response` MUST be beautifully formatted using Markdown. 
- Use **bold** for emphasis.
- Use lists/bullet points for summarizing multiple actions or research points.
- Make the final output look structured, professional, and visually premium for the user interface.
...
Return ONLY raw JSON matching the required Native Schema. DO NOT output free-text markdown outside the JSON. Ensure your markdown within the response string is properly escaped for JSON.
"""
...
def make_result(response, **kwargs):
    base = {
        ...
        "image_command": {}, "search_actions": {}, "youtube_play": "",
...
def run_agentic_loop(raw_command: str, context: str, memory_instance=None) -> Dict[str, any]:
    ...
    # Tool schema (UPDATED: added deep_research)
            ...
            if ai_response:
                for key, value in ai_response.items():
                    if key not in ignore_keys:
                        ...
                        if key == "search_actions" and isinstance(value, dict):
                            action_detail = value.get("web", "")
                            ...
                            action_detail = f"To: {value.get('params', {}).get('to', '')}"
                        ...
                        elif key == "apps_to_open" and isinstance(value, list):
                            action_detail = ", ".join(value)
                        elif key == "urls_to_open" and isinstance(value, list):
                            action_detail = ", ".join(value)
                        break

            # Actual thought aur extracted action update
            update_agent_status(
                step=step+1,
                total_steps=max_steps,
...
def process_with_cohere(raw_command: str, context: str, search_results: str = "", memory_instance=None) -> Dict[str, any]:
    ...
    required_keys = ["response", "apps_to_open", "apps_to_close", "urls_to_open", "image_command",
                     "search_actions", "youtube_play", "email_action", "workspace_action",
